...
SESSION=$(curl -s -f -H "Authorization: Bearer $STALWART_TOKEN" "$STALWART_URL")
...
MAILBOXES_RES=$(curl -s -X POST -H "Authorization: Bearer $STALWART_TOKEN" -H "Content-Type: application/json" -d '{
...
for obj in $OBJECTS; do
  ...
  BLOB_RES=$(curl -s -X POST -H "Authorization: Bearer $STALWART_TOKEN" -H "Content-Type: message/rfc822" --data-binary @"/tmp/$obj" "$UPLOAD_URL")
  ...
  IMPORT_RES=$(curl -s -X POST -H "Authorization: Bearer $STALWART_TOKEN" -H "Content-Type: application/json" -d "$JMAP_PAYLOAD" "$API_URL")
