...
locals {
  ...
  envbuilder_env = {
    ...
    "ENVBUILDER_GIT_SSH_KEY" : data.coder_workspace_owner.me.ssh_private_key,
...
resource "coder_agent" "main" {
  ...
  os             = "linux"
# MODIFY THIS: More robust permission handling
  startup_script = <<-EOT
    set -e
    ...
    # Wiping it allows envbuilder to try again cleanly on the next start.
    if [ -d "/workspaces/home-ops" ] && [ ! -d "/workspaces/home-ops/.git" ]; then
      echo "Cleaning up empty/failed clone directory..."
      rm -rf /workspaces/home-ops
    fi
  EOT
  dir            = "/workspaces"
